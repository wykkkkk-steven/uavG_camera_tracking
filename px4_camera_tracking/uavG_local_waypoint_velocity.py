...
# ================= Local waypoint tracking parameters =================
# A/B versions use the UAV's own local position + heading together with
# depth/image error to estimate a target-relative local waypoint.
KP_WAYPOINT = 0.45
TARGET_POSITION_FILTER_ALPHA = 0.35
TARGET_PREDICT_TIME_S = 0.30
POSITION_SETPOINT_UPDATE_S = 0.20
...
async def ask_track_duration(stop_event, land_event):
    print("\n[TARGET FOUND] 已找到目标。")
    print("现在请输入 A版 local waypoint velocity 跟踪飞行时长，单位秒。")
...
async def visual_orbit_control(
    ...
    print("\n[TRACK-A] 开始 local waypoint velocity tracking")
    print(f"[TRACK-A] 设定跟踪时长：{track_duration_s:.1f} s")
    print("[TRACK-A] 逻辑：估计 target_N/E → 生成 follow_N/E → 转成 body velocity")
    print("[TRACK-A] yaw 仍使用 error_x 保持目标居中；无 depth 时使用原 area fallback")
        ...
        if target_low or bbox_touch_bottom:
            ...
            if body_est is None or pos is None or heading_deg is None:
                ...
                if (
                    ...
                    target_n_used = filtered_target_n + target_v_n * TARGET_PREDICT_TIME_S
                    target_e_used = filtered_target_e + target_v_e * TARGET_PREDICT_TIME_S

                prev_target_n = filtered_target_n
                prev_target_e = filtered_target_e
                prev_target_time = now

                follow_n, follow_e, target_range = generate_follow_point(
                    drone_n,
                    drone_e,
                    target_n_used,
                    target_e_used,
                ...
                err_n = follow_n - drone_n
                err_e = follow_e - drone_e

                forward_cmd, right_cmd = local_ne_error_to_body_velocity(
                    err_n,
                    err_e,
                    heading_deg
                )
                ...
                forward_speed = clamp(
                    KP_WAYPOINT * forward_cmd,
                    -MAX_BACKWARD_SPEED,
                    MAX_FORWARD_SPEED
                )
                right_speed = clamp(
                    KP_WAYPOINT * right_cmd,
                    -MAX_FORWARD_SPEED,
                    MAX_FORWARD_SPEED
                )
                ...
                if abs_error_x > center_error_limit * 1.8:
                    forward_speed *= 0.50
                    right_speed *= 0.50
                    mode = "LOCAL_WAYPOINT_TURNING"
                else:
                    mode = "LOCAL_WAYPOINT"
                distance_source = "DEPTH+LOCAL"

                # User input orbit_right_speed is treated as an optional extra lateral bias.
                # For pure following, enter 0.
                right_speed = clamp(
                ...
                debug_extra = (
                    ...
                    f" follow_NE=({follow_n:+.1f},{follow_e:+.1f})"
