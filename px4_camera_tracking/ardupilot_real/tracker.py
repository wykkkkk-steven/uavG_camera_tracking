...
try:
    from . import config
    from .ap_ctrl import ArduPilotController
...
def publish_tracking_command(controller, forward, right, down, yaw_rate):
    """Pass the unchanged body command to the isolated ArduPilot adapter."""
    controller.update_command(
        forward,
        right,
        down,
        math.radians(float(yaw_rate)),
    )
...
def estimate_target_body_from_image(detector):
    ...
    The preserved tracking math currently assumes camera optical Z is aircraft
    forward and optical X is aircraft right. TODO(real hardware): apply the
    measured config.CAMERA_TO_BODY_ROTATION at this boundary. It is deliberately
    None because the real camera mount must not be guessed.
    """
...
async def main_async(dry_run):
    ...
    command_publisher = ArduPilotController(dry_run=dry_run)
    ...
    try:
        print(
            "ArduCopter adapter ready; arming, takeoff and landing remain "
            "under GCS/RC operator control."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Real ArduCopter ChArUco tracking controller"
    )
    ...
    mode.add_argument(
        ...
        dest="dry_run",
        action="store_true",
        help="print commands without transmitting MAVLink (default)",
    )
    mode.add_argument(
        "--live",
        dest="dry_run",
        action="store_false",
        help="transmit commands after all real-hardware configuration is set",
    )
