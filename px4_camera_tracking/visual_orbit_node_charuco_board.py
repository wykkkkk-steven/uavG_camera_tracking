...
async def wait_altitude_fast(drone, target=5.0, tol=0.3, timeout=10.0):
    """等待起飞高度；超时后提示并继续。"""
    t0 = time.time()
...
async def visual_orbit_control(drone, detector, track_duration_s, desired_area, approach_area, orbit_right_speed, desired_distance_m, stop_event, land_event):
    ...
    while not stop_event.is_set():
        ...
        if now - track_start > track_duration_s:
            print("[FINISH] 达到设定飞行时长，停止 tracking 并进入悬停等待指令")
            return "finished"
        ...
        # Limit how fast yaw_rate can change each control step.
