...
# ================= Reactive depth obstacle avoidance =================
# First-stage avoidance: use the existing depth image directly.
# No PointCloud2 bridge or map is required.
USE_REACTIVE_OBSTACLE_AVOIDANCE = True

# Only inspect the middle-height part of the image so the ground and sky/invalid
# pixels are less likely to dominate the obstacle measurement.
OBSTACLE_ROI_TOP_RATIO = 0.18
OBSTACLE_ROI_BOTTOM_RATIO = 0.72
OBSTACLE_DEPTH_PERCENTILE = 15.0
OBSTACLE_MIN_VALID_PIXELS = 80
OBSTACLE_MAX_RANGE_M = 12.0
OBSTACLE_DATA_TIMEOUT_S = 0.8

# Hysteresis: enter avoidance at the smaller distance and return to normal only
# after the centre region is clearly open again.
OBSTACLE_ENTER_DISTANCE_M = 2.5
OBSTACLE_CLEAR_DISTANCE_M = 3.2
OBSTACLE_EMERGENCY_DISTANCE_M = 1.25
OBSTACLE_SIDE_CLEAR_DISTANCE_M = 2.2

# During LEFT/RIGHT avoidance, use a short intermediate local-NED waypoint
# relative to the current aircraft pose.
AVOIDANCE_LATERAL_STEP_M = 1.50
AVOIDANCE_FORWARD_STEP_M = 0.20
AVOIDANCE_BODY_SPEED_M_S = 0.35

# Expand the target bbox before masking it out of obstacle processing.
TARGET_OBSTACLE_MASK_MARGIN_RATIO = 0.15
...
async def get_local_position_ned_once(drone, timeout_s=2.0):
    """Read PX4 local NED position used by PositionNedYaw."""
    try:
        pv = await asyncio.wait_for(
            drone.telemetry.position_velocity_ned().__anext__(),
            timeout=timeout_s
        )
        return (
            pv.position.north_m,
...
def estimate_target_body_from_image(detector):
    """Estimate target position in UAV body frame from depth and image position.

    Preferred calculation uses the camera intrinsics received from CameraInfo:
        right = (u - cx) * depth / fx

    If CameraInfo has not arrived yet, keep the original normalized-image
    approximation so the existing tracking behaviour is not interrupted.

    Assumptions kept from the original version:
    - camera is approximately facing forward
    - camera-right maps to body-right
    - depth gives approximate body-forward distance
    """
    ...
    if (
        detector.camera_fx is not None and
        detector.camera_cx is not None and
        detector.camera_fx > 1e-6 and
        detector.camera_info_width > 0
    ):
        # CameraInfo may be published for a different image resolution.
        # Scale the horizontal intrinsics to the actual RGB image size used by
        # the detector before applying the pinhole back-projection formula.
        scale_x = detector.image_w / float(detector.camera_info_width)
        fx_rgb = detector.camera_fx * scale_x
        cx_rgb = detector.camera_cx * scale_x

        target_right_body = (
            (detector.target_cx - cx_rgb)
...
def finite_distance_text(value):
    if value is None or not math.isfinite(value):
        return "None"
    return f"{value:.2f}"


def choose_obstacle_avoidance_state(detector, previous_state, now):
    """Return CLEAR, LEFT, RIGHT, STOP, or STALE.

    LEFT/RIGHT describe the direction the aircraft should move. The target bbox
    has already been masked out when the three obstacle distances were measured.
    """
    if not USE_REACTIVE_OBSTACLE_AVOIDANCE:
        return "CLEAR"

    if now - detector.last_obstacle_depth_time > OBSTACLE_DATA_TIMEOUT_S:
        return "STALE"

    center = detector.obstacle_center_m
    left = detector.obstacle_left_m
    right = detector.obstacle_right_m

    if center is None or not math.isfinite(center):
        return "CLEAR"

    # Keep an active avoidance manoeuvre until the centre has opened farther
    # than the clear threshold. This prevents rapid LEFT/CLEAR switching.
    threshold = (
        OBSTACLE_CLEAR_DISTANCE_M
        if previous_state in ("LEFT", "RIGHT", "STOP")
        else OBSTACLE_ENTER_DISTANCE_M
    )

    if center >= threshold:
        return "CLEAR"

    if center <= OBSTACLE_EMERGENCY_DISTANCE_M:
        return "STOP"

    left_open = (
        left is not None and math.isfinite(left) and
        left >= OBSTACLE_SIDE_CLEAR_DISTANCE_M
    )
    right_open = (
        right is not None and math.isfinite(right) and
        right >= OBSTACLE_SIDE_CLEAR_DISTANCE_M
    )
...
def avoidance_waypoint_from_body(
    drone_n,
    drone_e,
    heading_deg,
    avoidance_state,
):
    """Create a short safe intermediate waypoint in PX4 local N/E."""
    if avoidance_state == "STOP":
        return drone_n, drone_e

    if avoidance_state == "LEFT":
        right_body = -AVOIDANCE_LATERAL_STEP_M
    elif avoidance_state == "RIGHT":
        right_body = AVOIDANCE_LATERAL_STEP_M
    else:
        return None

    return body_to_local_ne(
        drone_n,
        drone_e,
        heading_deg,
        AVOIDANCE_FORWARD_STEP_M,
        right_body,
    )
...
async def wait_reached_offboard(
    ...
    while time.time() < deadline:
        ...
        local_pos = await get_local_position_ned_once(drone, timeout_s=1.5)
        if local_pos is None:
            continue

        north_now, east_now, down_now = local_pos
        north_err = north_tgt - north_now
        east_err = east_tgt - east_now
        down_err = down_tgt - down_now
...
async def return_home_and_hover(
    drone,
    home_north,
    home_east,
    home_down,
    ...
    target_north = home_north
    target_east = home_east
    target_down = home_down

    while time.time() < deadline:
        if stop_event.is_set() or land_event.is_set():
            return

        local_pos = await get_local_position_ned_once(drone, timeout_s=1.0)
        if local_pos is None:
            await drone.offboard.set_velocity_body(
                VelocityBodyYawspeed(0.0, 0.0, 0.0, 0.0)
            )
        ...
        down_err = target_down - down_now

        xy_dist = math.sqrt(north_err * north_err + east_err * east_err)

        if xy_dist < REACHED_XY_THR_M and abs(down_err) < REACHED_ALT_THR_M:
        ...
        # 如果高度已经接近目标，就不要再上下修正。
        if abs(down_err) < REACHED_ALT_THR_M:
            down_speed = 0.0
...
async def hover_wait_for_command(
    drone,
    stop_event,
    land_event,
    home_north,
...
class CharucoTargetDetector(Node):
    def __init__(self):
        ...
        # Reactive obstacle distances from the existing depth image.
        self.obstacle_left_m = None
        self.obstacle_center_m = None
        self.obstacle_right_m = None
        self.last_obstacle_depth_time = 0.0

        # Camera intrinsics from sensor_msgs/CameraInfo.
        # These are used for the standard pinhole back-projection formula.
        self.camera_fx = None
        self.camera_fy = None
        self.camera_cx = None
        self.camera_cy = None
    ...
    def camera_info_callback(self, msg):
        """Silently store the latest camera intrinsics and their resolution."""
        fx = float(msg.k[0])
        fy = float(msg.k[4])
        cx = float(msg.k[2])
        cy = float(msg.k[5])

        if fx <= 1e-6 or fy <= 1e-6 or msg.width <= 0 or msg.height <= 0:
            return
    ...
    @staticmethod
    def _robust_obstacle_distance(region):
        valid = region[np.isfinite(region)]
        valid = valid[valid > 0.25]
        valid = valid[valid < OBSTACLE_MAX_RANGE_M]

        if valid.size < OBSTACLE_MIN_VALID_PIXELS:
            return None

        return float(np.percentile(valid, OBSTACLE_DEPTH_PERCENTILE))

    def _update_obstacle_distances(self, depth, msg):
        """Measure left/centre/right obstacle distances from the depth frame."""
        height, width = depth.shape[:2]
        if width <= 0 or height <= 0:
            return

        y1 = int(height * OBSTACLE_ROI_TOP_RATIO)
        y2 = int(height * OBSTACLE_ROI_BOTTOM_RATIO)
        y1 = max(0, min(height - 1, y1))
        y2 = max(y1 + 1, min(height, y2))

        # Work on a copy because the target region is masked with NaN.
        roi_depth = depth.astype(np.float32, copy=True)

        if self.target_found and self.image_w > 0 and self.image_h > 0:
            scale_x = width / float(self.image_w)
            scale_y = height / float(self.image_h)
            ...
            margin_x = max(4, int(w * TARGET_OBSTACLE_MASK_MARGIN_RATIO))
            margin_y = max(4, int(h * TARGET_OBSTACLE_MASK_MARGIN_RATIO))

            mx1 = max(0, x - margin_x)
            mx2 = min(width, x + w + margin_x)
            my1 = max(0, y - margin_y)
            my2 = min(height, y + h + margin_y)

            if mx2 > mx1 and my2 > my1:
                roi_depth[my1:my2, mx1:mx2] = np.nan

        x_left_end = int(width * 0.34)
        x_center_start = int(width * 0.33)
        x_center_end = int(width * 0.67)
        x_right_start = int(width * 0.66)

        left_region = roi_depth[y1:y2, 0:x_left_end]
        center_region = roi_depth[y1:y2, x_center_start:x_center_end]
        right_region = roi_depth[y1:y2, x_right_start:width]

        self.obstacle_left_m = self._robust_obstacle_distance(left_region)
        self.obstacle_center_m = self._robust_obstacle_distance(center_region)
        self.obstacle_right_m = self._robust_obstacle_distance(right_region)
        self.last_obstacle_depth_time = time.time()
    ...
    def depth_callback(self, msg):
        ...
        # Obstacle processing uses the full depth image and does not require a
        # target. When a target exists, its expanded bbox is masked out.
        self._update_obstacle_distances(depth, msg)

        # Target-distance extraction still requires a selected RGB target.
...
def reset_detector_tracking_state(detector):
    ...
    detector.obstacle_left_m = None
    detector.obstacle_center_m = None
    detector.obstacle_right_m = None
    detector.last_obstacle_depth_time = 0.0
...
async def visual_orbit_control(
    ...
    while not stop_event.is_set():
        ...
        avoidance_state = choose_obstacle_avoidance_state(
            detector,
            avoidance_state,
            now,
        )
        ...
        if not recently_seen:
            ...
            if (
                ...
                # Do not continue toward an old waypoint while a close obstacle
                # is visible and the target itself has been lost.
                if avoidance_state in ("LEFT", "RIGHT", "STOP"):
                    await drone.offboard.set_velocity_body(
                        VelocityBodyYawspeed(0.0, 0.0, 0.0, 0.0)
                    )
                else:
        ...
        if (target_low or bbox_touch_bottom) and not position_tracking_active:
            ...
            local_pos = await get_local_position_ned_once(drone, timeout_s=0.7)
            heading_deg = await get_heading_once(drone, timeout_s=0.5)

            if body_est is None or local_pos is None or heading_deg is None:
                ...
                drone_n, drone_e, _drone_down = local_pos
                ...
                # Reactive safety layer. A blocked centre ROI replaces the raw
                # tracking follow point with a short intermediate waypoint.
                avoidance_waypoint = avoidance_waypoint_from_body(
                    drone_n,
                    drone_e,
                    heading_deg,
                    avoidance_state,
                ...
                if avoidance_waypoint is not None:
                    follow_n_raw, follow_e_raw = avoidance_waypoint
                ...
                if avoidance_state == "LEFT":
                    mode = "AVOID_LEFT_WAYPOINT"
                    distance_source = "DEPTH+OBSTACLE"
                elif avoidance_state == "RIGHT":
                    mode = "AVOID_RIGHT_WAYPOINT"
                    distance_source = "DEPTH+OBSTACLE"
                elif avoidance_state == "STOP":
                    mode = "AVOID_STOP_WAYPOINT"
                    distance_source = "DEPTH+OBSTACLE"
                else:
                ...
                debug_extra = (
                    ...
                    f" heading={heading_deg:.1f}"
                    f" avoid={avoidance_state}"
                    f" obs_LCR=("
                    f"{finite_distance_text(detector.obstacle_left_m)},"
                    f"{finite_distance_text(detector.obstacle_center_m)},"
                    f"{finite_distance_text(detector.obstacle_right_m)})"
                )
            ...
            # In area fallback, obstacle avoidance overrides forward tracking.
            if avoidance_state == "LEFT":
                mode = "AVOID_LEFT_VELOCITY"
                distance_source = "DEPTH+OBSTACLE"
                forward_speed = 0.0
                right_speed = -AVOIDANCE_BODY_SPEED_M_S
            elif avoidance_state == "RIGHT":
                mode = "AVOID_RIGHT_VELOCITY"
                distance_source = "DEPTH+OBSTACLE"
                forward_speed = 0.0
                right_speed = AVOIDANCE_BODY_SPEED_M_S
            elif avoidance_state == "STOP":
                mode = "AVOID_STOP_VELOCITY"
                distance_source = "DEPTH+OBSTACLE"
                forward_speed = 0.0
                right_speed = 0.0
...
async def main_async():
    ...
    try:
        ...
        # Record this run's home directly in the same PX4 local NED frame
        # used by PositionNedYaw.
        local_home = await get_local_position_ned_once(drone, timeout_s=2.0)
        if local_home is None:
            raise RuntimeError("无法读取 PX4 local NED，不能建立 HOME 参考点")

        home_north, home_east, home_down = local_home
        pos = await drone.telemetry.position().__anext__()
        print(
            f"[HOME参考点] local_NED=({home_north:+.2f},"
            f"{home_east:+.2f},{home_down:+.2f})m "
            f"lat={pos.latitude_deg:.7f} lon={pos.longitude_deg:.7f} "
